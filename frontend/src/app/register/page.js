...
import { Mail, KeyRound, User, Loader2, CheckCircle2 } from "lucide-react";
  ...
  const verifyOtp = async (e) => {
    ...
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000") + "/api/auth/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, otp, name, type: "register" }),
      });
            ...
            <AnimatePresence mode="wait">
              {step === 0 ? (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                >
                  <div className="mb-8 border-l-2 border-[var(--color-neon-orange)] pl-4">
                    <h1 className="text-2xl font-bold text-white mb-1 font-mono tracking-tight">
                      <span className="text-[var(--color-neon-orange)] mr-2">&gt;</span>
                      Sign Up
                    </h1>
                    <p className="text-gray-400 text-sm font-inter">Deploy your FinVest workspace.</p>
                  </div>
                  ...
                  <form onSubmit={requestOtp} className="space-y-5">
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-300 ml-1 font-inter">Full Name</label>
                      <div className="relative group">
                        <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-gray-500 group-focus-within:text-[var(--color-neon-orange)] transition-colors">
                        ...
                        <input 
                          ...
                          className="w-full bg-black/50 border border-gray-800 rounded-lg py-3 pl-11 pr-4 text-white text-sm font-mono outline-none focus:border-[var(--color-neon-orange)] focus:bg-[var(--color-neon-orange)]/5 transition-all placeholder:text-gray-600"
                    ...
                    <button 
                      type="submit"
                      disabled={loading}
                      className="w-full py-3 mt-6 bg-[var(--color-neon-orange)] text-black font-bold rounded-lg hover:bg-white transition-colors flex items-center justify-center gap-2 disabled:opacity-50 text-sm font-inter shadow-[0_0_15px_rgba(255,100,0,0.2)]"
                    >
                  ...
                  <div className="mt-8 text-center border-t border-gray-800 pt-6">
                    <p className="text-sm text-gray-500 font-inter">
                      Already have an account? <Link href="/login" className="text-white hover:text-[var(--color-neon-orange)] transition-colors hover:underline underline-offset-4">Sign in</Link>
                    </p>
                  </div>
                </motion.div>
              ) : (
                ...
                >
                  ...
                  <form onSubmit={verifyOtp} className="space-y-6">
                    <div className="space-y-3">
                      <label className="text-xs font-mono text-[var(--color-neon-orange)] uppercase tracking-widest ml-1 flex items-center justify-between">
                      ...
                      <div className="relative group">
                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-[var(--color-neon-orange)]">
                        ...
                        <input 
                          ...
                          className="w-full bg-[#050505] border border-[var(--color-neon-orange)]/50 rounded-lg py-4 pl-12 pr-4 text-[var(--color-neon-orange)] text-2xl font-mono outline-none focus:border-[var(--color-neon-orange)] focus:shadow-[0_0_20px_rgba(255,100,0,0.2)] transition-all placeholder:text-[var(--color-neon-orange)]/20 tracking-[0.5em] text-center"
                          placeholder="000000"
                        />
                      </div>
                    </div>
